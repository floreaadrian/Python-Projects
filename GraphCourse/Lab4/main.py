...
def longestPath(g, x, y, list):
    dp = [-10000000]*(g.number_of_vertices+1)
    dp[x] = 0
    for el in list:
        if dp[el] == -10000000:
            continue
        for node in g.parseNout(el):
            dp[node[0]] = max(dp[node[0]], dp[el]+node[1])
    t = y
    path = []
    if dp[y] != -10000000:
        while dp[t] != 0:
            path.append(t)
            for j in g.parseNin(t):
                if dp[j[0]] + j[1] == dp[t]:
                    t = j[0]
                    break
        path.append(t)
...
def makeGraph(g, data):
    ...
    for i in range(1, data[0][1] + 1):
        if len(data[i]) == 3:
            ...
            g.addEdge(data[i][0]-1, data[i][1]-1, 0)
   # g.sorte()


data = readData("graph0.txt")
g = Graph(data[0][0])
makeGraph(g, data)
...
while True:
    ...
    if cmd == '1':
        ...
        if len(result) == g.lenghtGraph():
            ...
            x = input("Source: ")
            y = input("Destination: ")
            print(longestPath(g, int(x), int(y), result))
        else:
            print("No")
    elif cmd == '0':
        break
